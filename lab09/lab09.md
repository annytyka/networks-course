...
1. Количество отправленных пакетов: $\frac{W}{2} + \Big( \frac{W}{2} + 1 \Big) + \ldots + W = \frac{\Big( \frac{W}{2} + W \Big) \Big( \frac{W}{2} + 1 \Big)}{2} = \frac{3}{8} W^2 + \frac{3}{4} W$.

Потерян один пакет, поэтому частота потерь равна $L = \frac{1}{\frac{3}{8} W^2 + \frac{3}{4} W}$.

2. Можно пренебречь линейным членом, тогда из $L \approx \frac{1}{\frac{3}{8} W^2 + \frac{3}{4} W}$ получаем $W \approx \sqrt{\frac{8}{3L}}$.

Среднее окно равно $\frac{\frac{W}{2} + W}{2} = \frac{3W}{4}$. 

Средняя скорость: $\frac{3W}{4} \cdot \frac{MSS}{RTT} \approx \frac{3}{4} \cdot \sqrt{\frac{8}{3L}} \cdot \frac{MSS}{RTT} = \frac{1.22 \cdot MSS}{RTT \cdot \sqrt{L}}$.
...
1. В предыдущей лабораторной было такое же мультипликативное увеличение, только в $1 + a$ раз. Перепишем для других обозначений (в $a$ раз).

Сегменты будут отправляться по $\frac{W}{2} a^k$ на $k$-том шаге (начиная с нулевого) до достижения $W$. Тогда количество сегментов выражается так: $\frac{W}{2} (1 + a + a^2 + \ldots + a^{log_a 2}) = \frac{W}{2} \cdot \frac{a^{log_a 2 + 1} - 1}{a - 1} = \frac{W}{2} \cdot \frac{2a - 1}{a - 1}$.

$L = \frac{1}{\frac{W}{2} \cdot \frac{2a - 1}{a - 1}} = \frac{2(a - 1)}{W(2a - 1)} = \frac{2(a - 1)}{2a - 1} \cdot \frac{1}{W}$.

2. Последнее количество отправленных пакетов равно $\frac{W}{2} \cdot a^{log_a 2}$, первое равно $\frac{W}{2}$, поэтому время увеличения окна равно $RTT \cdot log_a 2$, что не зависит от средней пропускной способности.
