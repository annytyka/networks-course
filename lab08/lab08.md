...
Среднее окно и средняя пропускная способность $R = \frac{W + \frac{W}{2}}{2} = \frac{3W}{4}$, $T = 1 - \frac{W}{2} = \frac{W}{2}$. 

Из первого $W = \frac{4R}{3}$, из второго $T = \frac{2R}{3}$, что действительно является функцией от $R$.
...
$15 = 1 + 2 + 4 + 8$, сегменты передаются так. Один сегмент передаётся за $t = \frac{S}{R}$. 

Время без учёта задержек: $T = 2RTT + 15t$.

1. Задержки после отправки одного и двух сегментов. $T + RTT + (RTT - t) = 4RTT + 14t$.
2. Задержки после отправки одного, двух и четырёх сегментов. $T + RTT + (RTT - t) + (RTT - 3t) = 5RTT + 11t$.
3. Задержки после отправки одного сегмента. $T + RTT = 3RTT + 15t$.
...
Сегменты будут отправляться по $\frac{W}{2} (1 + a)^k$ на $k$-том шаге (начиная с нулевого) до достижения $W$. Тогда количество сегментов выражается так: $\frac{W}{2} (1 + (1 + a) + (1 + a)^2 + \ldots + (1 + a)^{log_{1 + a} 2}) = \frac{W}{2} \cdot \frac{(1 + a)^{log_{1 + a} 2 + 1} - 1}{a} = \frac{W}{2} \cdot \frac{2(a + 1) - 1}{a} = \frac{W}{2} \cdot \frac{2a + 1}{a}$.

$L = \frac{1}{\frac{W}{2} \cdot \frac{2a + 1}{a}} = \frac{2a}{W(2a + 1)}$.
...
1. $RTT_{FE}$ — установление TCP-соединения между клиентом и внешним сервером.
2. $RTT_{BE}$ — передача по уже установленному соединению между внешним и внутренним сервером и обратно.
3. $3RTT_{FE}$ — передача от внешнего сервера клиенту в фазе медленного старта.

Действительно, в сумме $4 \cdot RTT_{FE} + RTT_{BE} + \text{ время обработки}$.
